...
class AssetController(
    ...
    fun tickerInfo(@RequestParam ticker: String): String {
        ...
        val typeOptions = ASSET_TYPES.joinToString("") { t ->
    ...
    fun editAsset(
        ...
        transaction {
            val asset = AssetEntity.findById(normalizedTicker)
    ...
    fun deleteAsset(@PathVariable ticker: String): String {
        val normalizedTicker = ticker.uppercase()
        transaction {
            val asset = AssetEntity.findById(normalizedTicker)
                ?: throw ResponseStatusException(HttpStatus.NOT_FOUND, "Asset not found")
    ...
    @PostMapping("/api")
    @ResponseBody
    fun createAssetApi(@RequestBody request: AssetRequest): ResponseEntity<AssetResponse> {
        request.validate()
    ...
    @ResponseBody
    fun getAssetApi(@PathVariable ticker: String): AssetResponse {
        return transaction {
    ...
    private fun AssetEntity.toResponse() = AssetResponse(
        ticker = ticker.value,
        yfTicker = yfTicker,
        name = name,
        type = type,
